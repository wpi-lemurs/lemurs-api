...
def fetch_all_users() -> List[Dict[str, Any]]:
    """Return ALL users with their start date and danger alert status."""
    sql = f"""
    SELECT
        u.app_user_id,
        u.umass_id,
...
app = Dash(
    __name__,
    server=server,
    requests_pathname_prefix="/",   # serve at root on port 5433
    routes_pathname_prefix="/",     # Dash routes live at /
    serve_locally=True,             # serve component bundles from this server
...
def set_embed_headers(resp):
    # Prefer modern CSP frame-ancestors over X-Frame-Options
    csp = "frame-ancestors 'self' http://localhost:3000 http://127.0.0.1:3000;"
    existing = resp.headers.get("Content-Security-Policy")
    # Keep any existing directives but ensure frame-ancestors is present/overridden
    if existing:
        # naive merge: replace any prior frame-ancestors directive
        parts = [p.strip() for p in existing.split(";") if p.strip() and not p.strip().startswith("frame-ancestors")]
        parts.append(csp.rstrip(";"))
        resp.headers["Content-Security-Policy"] = "; ".join(parts) + ";"
    else:
        resp.headers["Content-Security-Policy"] = csp
    # Remove X-Frame-Options if present (it conflicts with embedding)
...
def update_display(load_clicks, active_cell, user_id_from_input, table_data):
    ...
    if user_id_to_fetch:
        ...
        if not survey_dates:
            ...
            fig = calplot(df, x="timestamp", y="count", title="Survey Activity", colorscale="teal")
            fig.update_layout(title_x=0.5)
            plot_content = dcc.Graph(figure=fig)

        return data, columns, plot_content, status, debug_msg
